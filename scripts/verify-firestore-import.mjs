...
const CLIENTS_CSV = "db-audit/out/client-import/clients-import.crm.csv";
const APPOINTMENTS_CSV = "db-audit/out/final-import/appointments-import.crm.csv";
...
async function main() {
  ...
  const appointments = parseCsv(fs.readFileSync(APPOINTMENTS_CSV, "utf8"));
  const db = await loadAdmin(process.env.FIREBASE_PROJECT_ID || process.env.GCLOUD_PROJECT);

  const result = {
    clients: await countExisting(db, "clients", clients.map((row) => row.id)),
    appointments: await countExisting(db, "appointments", appointments.map((row) => row.id)),
  };

  fs.writeFileSync("db-audit/out/firestore-import/firestore-post-import-verification.json", JSON.stringify(result, null, 2), "utf8");
  console.log(JSON.stringify(result, null, 2));
  if (result.clients.missingCount > 0 || result.appointments.missingCount > 0) process.exitCode = 1;
}
