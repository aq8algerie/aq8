...
type CenterBookingFormProps = {
  ...
  centerCity?: string;
  services?: string[];
  center?: Pick<Center, "id" | "bookingCapacity" | "bookingHours" | "services" | "name" | "city">;
  allCenters?: Center[];
};
...
function getServiceDescription(service: string) {
  if (!service) return "";
  if (service.toLowerCase() === "aq8") return "Séance active d'électrostimulation globale pour tonifier et raffermir.";
  if (service.toLowerCase() === "wonder") return "Technologie d'ondes électromagnétiques pour cibler les zones rebelles.";
  return "";
}
...
export function CenterBookingForm({
  ...
  centerName: propCenterName,
  centerCity: propCenterCity,
  services: propServices = [],
  center: propCenter,
  allCenters,
}: CenterBookingFormProps) {
  const [selectedCenterId, setSelectedCenterId] = useState<string>(propCenterId || "");
  ...
  const activeCenter = useMemo(() => {
    ...
    if (allCenters && selectedCenterId) {
      return allCenters.find(c => c.id === selectedCenterId);
    }
    return undefined;
  ...
  const defaultDate = useMemo(() => {
    ...
    return getNextOpenBookingDate(resolvedCenterId, bookingMinimumDate, 45, bookingCenter);
  }, [resolvedCenterId, bookingMinimumDate, bookingCenter]);
  ...
  const hours = useMemo(() => {
    if (!resolvedCenterId || !bookingDate) return [];
    return getBookingHoursForDate(resolvedCenterId, bookingDate, bookingCenter);
  }, [resolvedCenterId, bookingDate, bookingCenter]);

  const capacitySummary = useMemo(() => {
    if (!resolvedCenterId) return "";
    return getCapacitySummary(resolvedCenterId, bookingCenter);
  }, [resolvedCenterId, bookingCenter]);

  const hourAvailability = useMemo(() => {
    const next: Record<string, SlotAvailability> = {};
    if (!resolvedCenterId) return next;
    for (const hour of hours) {
      next[hour] = resolveSlotAvailability(resolvedCenterId, selectedServiceType, bookingCenter, slotByTime[hour]);
    }
    return next;
  }, [bookingCenter, resolvedCenterId, hours, selectedServiceType, slotByTime]);
  ...
  useEffect(() => {
    setSlotByTime({});
    if (!resolvedCenterId || !bookingDate) {
      setAvailabilityLoading(false);
      return;
    }

    setAvailabilityLoading(true);
    const slotsRef = collection(db, "public_booking_slots", resolvedCenterId, "slots");
    ...
    return onSnapshot(
      ...
      (snapshot) => {
        ...
        snapshot.forEach((doc) => {
          const slot = { id: doc.id, ...doc.data() } as PublicBookingSlot;
          if (slot.time) {
            next[slot.time] = slot;
          }
        });
      ...
      (error) => {
        console.error("Public booking slots query error:", error);
        setAvailabilityLoading(false);
      },
    );
  }, [resolvedCenterId, bookingDate]);
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setPhone("");
      setEmail("");
      setService(resolvedServices[0] || "aq8");
      const nextDate = getNextOpenBookingDate(resolvedCenterId, getBookingMinimumDate(), 45, bookingCenter);
      setBookingDate(nextDate);
      setBookingTime(getBookingHoursForDate(resolvedCenterId, nextDate, bookingCenter)[0] || "");
    } catch (err) {
  ...
  return (
    <div className="rounded-lg border border-slate-200/80 bg-white p-6 shadow-[0_24px_60px_rgba(255,87,87,0.06)] relative overflow-hidden ring-1 ring-slate-100/50">
      ...
      {!propCenterId && allCenters && allCenters.length > 0 && (
        <div className="mb-5 space-y-1.5 text-sm font-semibold text-slate-700">
          <label className="text-slate-600 block font-bold">Choisissez votre centre AQ8 *</label>
          <select
