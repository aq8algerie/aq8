...
export async function POST(request: Request) {
  try {
    ...
    if (!(file instanceof File) || !postId) {
      throw new CrmAccessError('Fichier ou identifiant éditorial manquant.', 400);
    }
    if (!['cover', 'content'].includes(usage)) {
      throw new CrmAccessError('Usage de l’image invalide.', 400);
    }
    if (!/^[a-zA-Z0-9_-]{3,120}$/.test(postId)) {
      throw new CrmAccessError('Identifiant éditorial invalide.', 400);
    }
    if (!ALLOWED_MIME_TYPES.has(file.type)) {
      throw new CrmAccessError('Formats acceptés : JPG, PNG ou WebP.', 400);
    }
    if (file.size <= 0 || file.size > MAX_IMAGE_BYTES) {
      throw new CrmAccessError('L’image doit peser moins de 6 Mo.', 400);
    }
    ...
    const destinationPath = `blog/${postId}/${usage}/${randomUUID()}-${sanitizeFileName(file.name)}`;
    ...
    console.error('[upload-blog-image] failed:', error);
